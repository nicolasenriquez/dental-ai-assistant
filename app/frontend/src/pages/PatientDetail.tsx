...
import { type EvolutionSummary, type Patient, getPatient, getPatientEvolutions } from '../lib/api';
  ...
  return (
    <main className="min-h-full bg-[var(--bg)] text-[var(--text-primary)] p-6 md:p-8">
      <div className="max-w-6xl mx-auto">
        <Link to="/patients" className="text-sm text-[var(--accent)] hover:underline">
          ‹ Pacientes
        </Link>
        {loading ? (
          ...
          <>
            <header className="flex flex-wrap items-start justify-between gap-4 mt-4">
              <div>
                ...
                <p className="text-sm text-[var(--text-secondary)] mt-1">
                  ...
                  {patient.birth_date
                    ? ` · Nacimiento ${new Date(`${patient.birth_date}T00:00:00`).toLocaleDateString('es-CL')}`
                    : ''}
                </p>
              </div>
              <Link
                to={`/patients/${patient.id}/evolutions/new`}
                className="px-4 py-2 rounded-lg bg-[var(--accent)] text-white focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:outline-none"
              >
                + Nueva evolucion
              </Link>
            </header>
            <h2 className="text-xs font-semibold tracking-wider text-[var(--text-secondary)] mt-8">
            ...
            {evolutions.length === 0 ? (
              ...
              <ul className="mt-3 divide-y divide-[var(--border)] rounded-lg border border-[var(--border)] bg-[var(--surface-1)]">
                {evolutions.map((evolution) => (
                  <li key={evolution.id}>
                    <Link
                      to={`/patients/${patient.id}/evolutions/${evolution.id}`}
                      className="block p-4 hover:bg-[var(--surface-2)] focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--accent)] focus-visible:outline-none"
                    >
