...
so it can be reviewed (by the `review-pr` agentic gate, then a human).
...
- **Implementation report** (if `execute` wrote one — `.claude/reports/<…>-report.md`): pull the summary,
...
Report the PR number + URL, the base ← head branches, and **"Ready for review → run `review-pr <number>`, then a
