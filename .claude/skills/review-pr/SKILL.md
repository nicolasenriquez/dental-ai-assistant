...
- **The implementation report** (if `execute` wrote one — `.claude/reports/*{branch}*`) + its plan: read the
...
Run the project's real suite (the **`validate`** skill, or the plan's validation commands) — tests, type-check,
...
next step is **`code-review-fix`** on the report, then re-run validation.
