...
- Store only an AES-256-GCM-encrypted refresh token plus managed-folder identity in PostgreSQL. Keep access tokens in bounded process memory only, except for a short-lived no-store Picker response returned to the current browser.
- Create exactly one visible `Dental AI Assistant` folder for the current Google connection and identify it by persisted `folder_id` plus private `appProperties`, never by name lookup.
- Manage only UTF-8 `.md` and `.txt` blob files no larger than 1 MiB. Every managed file belongs to the managed folder, carries private application metadata, and binds to one patient owned by the Dental user.
- Add cursor-paginated managed-file listing and body-based server search scoped to the active patient. Document names never enter URL queries or logs.
- Import external `.md` or `.txt` files only through Google Picker by creating a new patient-bound copy inside the managed folder. Never retain, move, update, or delete the source file.
...
- Block active-patient switching while a Drive document is dirty until the user successfully saves, explicitly discards, or cancels the switch.
...
- Persisting access tokens, file content, source Picker IDs, local clinical drafts, or a duplicate Drive file catalog in PostgreSQL.
...
- Adds migration `0011_google_drive_workspace`, `db/google_drive_repo.py`, Google OAuth/Drive integrations, token cipher, route module, and focused backend tests.
...
- Adapter: `asyncpg`, `httpx.AsyncClient`, AES-GCM token cipher, Google Picker React adapter, Radix/shadcn primitives, and `app/frontend/src/lib/api.ts`.
...
- First failing behavior or contract proof: an authenticated user cannot connect without valid OAuth state; plaintext refresh tokens never reach persistence; only the managed patient-bound folder/file set is readable or writable; source imports create copies; stale versions return `409`; Drive insertion changes only composer draft; Assistant output opens only a local Drive draft; dirty patient switching requires resolution; and no token, document title, or clinical content enters logs.
...
- Prove negative security boundaries directly, including foreign user, foreign patient, foreign folder, missing metadata, wrong MIME, oversize content, revoked grant, state mismatch, and version conflict.
...
- Boundaries: Google Drive is source of truth for managed file metadata/content; PostgreSQL stores only connection credentials and managed-folder identity; every transfer to Chat or Drive remains visible, editable, and explicitly confirmed by the professional.
