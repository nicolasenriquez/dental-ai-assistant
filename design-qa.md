...
| Patient status panel | Native Chrome via CDP, 1440 × 1000, 1024 × 900, 390 × 844 | Wide anchored patient context directly above composer | Panel stays composer-width, exposes only masked patient context, remains open while typing, closes with Escape, and has no horizontal overflow | Draft initially reset when crossing the mobile breakpoint because the workspace remounted; stable sibling keys fixed it | medium | fixed and regression-tested |
...
- Native Chrome Google session: authenticated manually, selected the real patient, opened and dismissed the patient status panel, and checked 1440 × 1000, 1024 × 900, and 390 × 844. The pass exposed a draft reset at the 767 px breakpoint. After keying the `AppShell` workspace siblings, `Control en seis meses` remained in the composer at 390 × 844 and horizontal overflow remained false. Console entries came from the unauthenticated `/api/auth/me` probe and Google local-origin/COOP flow; no clinical runtime error appeared after login.
